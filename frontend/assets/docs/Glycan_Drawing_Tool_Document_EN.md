...
The current version adds the following tools and interface improvements:

- **Render Presets** --- Each sugar can use **Flat**, **Soft**, or **Glossy** rendering. Mixed selections show no pressed render button.
- **Add-Mode Linkage Tab** --- Configure linkage text, line style, label style, and direction before creating a new connection.
- **Rotation and Alignment** --- Use the compass dial, numeric angle input, or preset angle buttons. Alignment, branch nudging, and linkage-length normalization are also available in the View tab.
- **SVG Clipboard Copy** --- `Ctrl+Alt+C` copies selected sugars, texts, and connections as an SVG image for pasting into Word, PowerPoint, or other applications.
- **Add-Mode Double Click** --- Double-click an existing sugar while in Add Sugar mode to switch to Select mode and adjust that sugar.
- **Tool Shortcuts** --- `Alt+A` Add Sugar, `Alt+T` Add Text, `Alt+S` Select, `Alt+P` Preset Glycans, `Alt+D` Delete.
- **Visitor Statistics** --- The left panel shows Busuanzi page views and visitors, with labels that follow the active language.
- **Font and Version Display** --- Chinese uses Noto Sans SC, English uses Novecento Sans with a Google font fallback. The current version number appears beside the title.
